...
calcIO <- function(subtype = c("input", "output", "trade"),
  ...
  if (corrected) {
    ...
    if (subtype %in% c("output", "input")) {
      ...
      reminditems <- mbind(
        reminditems,
        setNames(
          reminditems[, , "pebiolc.sesobio.biotr"] * (1 - shareBiotrad),
      ...
      feShares <- new.magpie(
        cells_and_regions = regions, years = intersect(getYears(share), getYears(reminditems)),
        names = c(
          "seliqfos.fepet.tdfospet", "seliqbio.fepet.tdbiopet", "seliqfos.fedie.tdfosdie",
          "seliqbio.fedie.tdbiodie"
        )
      )
      feShares[regions, getYears(feShares), "fepet"] <- setNames(
        share[regions, getYears(feShares), "share_LDV_totliq"],
        NULL
      )
      ...
      reminditems[, getYears(feTransp), c(
        "seliqfos.fepet.tdfospet", "seliqbio.fepet.tdbiopet",
  ...
  desc <- paste0("IEA SE Output Data based on ", toolGetIEAYear(ieaVersion),
