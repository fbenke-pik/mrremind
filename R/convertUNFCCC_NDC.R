  ...
  if (grepl("Capacity", subtype, fixed = TRUE)) {
    ...
    calcGhgFactor <- function(data) {
      ...
      if (nregions(data) != 1 || nyears(data) != 1) {
        stop("function calcGhgFactor(data) should be called with one single region and year as data only")
      }
      ...
      if (allowedType[data[regi, year, "Type"]] == "GHG-Absolute") {
        if (data[regi, year, "Reference_Year"] == -1) {  # -1 if BAU;
          if (!is.na(data[regi, year, "BAU_or_Reference_emissions_in_MtCO2e"])) {
            # target + BAU emissions
            ghgTarget <- data[regi, year, conditional] +
              data[regi, year, "BAU_or_Reference_emissions_in_MtCO2e"]
          } else {
            # TODO: how to handle missing BAU emissions?
            warning("For ", regi, " in ", year, ", BAU Emissions are missing.")
          }
        } else { # use historic ghg data for Reference_Year, not those given in the database
          ghgTarget <- setYears(ghg[regi, min(data[regi, year, "Reference_Year"], max(getYears(ghg, as.integer = TRUE))), ],
                                NULL) + data[regi, year, conditional]
        }
        ...
        if (data[regi, year, "Reference_Year"] == -1) {  # -1 if BAU.
          ...
          # target * ghg in reference year (approximate when not available in 'ghg')
          ghgTarget <- (1 + data[regi, year, conditional]) *
        ...
        # TODO: this seems to be inaccurate for CO2/GDP
        ...
        if (!is.na(reductionData[regi, year, conditional][1])) {
          ...
          if (isTRUE(ghgFactorMin < 0) && !year %in% knownLow[[regi]]) {
            stop(
              ...
              "Is the country really promising negative net emissions? Add it to 'knownLow' in convertUNFCCC_NDC"
            )
          }
        }
      }
    ...
    for (year in getYears(reductionData)) { # assign EUR NDC goal to all EUR28 countries
      if (!is.na(reductionData["EUR", year, conditional])[1]) { # check whether a target exists
        if (allowedType[reductionData["EUR", year, "Type"]] == "GHG") {
          ghggoal <- (1 + reductionData["EUR", year, conditional]) * setYears(ghg[EUR_NDC_countries, min(reductionData["EUR", year, "Reference_Year"], max(getYears(ghg, as.integer = TRUE))), ], NULL)
          ...
          ghgFactor[EUR_NDC_countries, year, ] <- reductionData["EUR", year, conditional] / ghg_EUR_2005
        } else {
          warning(
